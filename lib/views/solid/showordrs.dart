          ...
          try {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                if (docData != null && docData["pickup_time"] != "" && docData["date_requested"] != "") {
                  String customerName = docData["customer_name"] ?? "غير معروف";
                                ...
                                onPressed: docData["receivedTime"] != ""
                                    ...
                                    : () async {
                                        await FirebaseFirestore.instance
                                            .collection("orders")
                                            .doc(docData["order_id"])
                                            .update({
                                          "receivedTime": FieldValue.serverTimestamp(),
                                        });

                                        // Make driver available again
                                        if (docData["d_id"] != null && docData["d_id"].toString().isNotEmpty) {
                                          await FirebaseFirestore.instance
                                              .collection("Delivers")
                                              .doc(docData["d_id"])
                                              .update({
                                        ...
                                        final cDoc = await FirebaseFirestore.instance
                                            ...
                                            .doc(docData["customer_id"])
                                            .get();
                                        
                                        String token = cDoc.data()?["token"] ?? "";
                                        if (token.isNotEmpty) {
                                          AuthController.sendPushMessage(
                                              token,
                                              "عزيزي العميل / تم إيصال عينتك الى مختبرات ألفا\nالساعة:${DateFormat(' hh:mm:ss ').format(docData["pickup_time"].toDate())} ${(DateFormat('a').format(docData["date_requested"].toDate()) == 'AM') ? 'صباحًا' : 'مساءً'}\n  شاكرين تعاملكم وثقتكم بنا",
                                              "إشعار وصول العينة",
                                              data: {'order_id': docData["order_id"] ?? '', 'type': 'sample_received'});
                                        }
                                        showCustomSnackBar("تم استلام  الطلب بنجاح");
                                      }),
                          ),
                          ListTile(
                            title: ElevatedButton(
