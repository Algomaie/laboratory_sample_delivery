...
class addcustome extends StatefulWidget {
  @override
  _addcustomeState createState() => _addcustomeState();
}

class _addcustomeState extends State<addcustome> {
  ...
  void _register(AuthController authController) async {
    ...
    if (name.isEmpty) {
      showCustomSnackBar('ادخل اسم العميل'.tr);
      ...
      CusttomerModel del = CusttomerModel.withId(
          id: docRef.id,
          dname: name,
          address: address,
          email: email,
      ...
      await docRef.set(del.toMap()).then((value) {
        showCustomSnackBar("تم اضافة عميل بنجاح", isError: false);
      }).catchError((error) {
